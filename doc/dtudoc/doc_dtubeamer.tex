...
\frame{
	...
	\begin{tcolorbox}[title={Framed blocks}]
...
%		\end{minipage} 
   ...
   \hfill 
   \begin{minipage}[l]{0.49\textwidth}
   \begin{tcolorbox}[colback=white, colframe=dtured, title={Custom block}]
...
\frame{
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \textbf{Windows:} include the \texttt{verdana} package in the preamble and MiKTeX will do the rest. Remember to set the \texttt{\textbackslash usepackage[nofligs]\{verdana\}} option due to missing ligatures in new versions of Microsofts Verdana font.
