...
% 
% 2) Verdana font type
%\usepackage{arev}             % Vera Sans for maths and typewriter
%\usepackage[nofligs]{verdana} % Overwrite normal text with real Verdana
% 
% 3) Helvetica (Arial's big brother)
%\usepackage{helvet}
% 
% 4) Times (text and math)
%\usepackage{newtxtext, newtxmath}
%
% 5) Another nice font combination
\usepackage{lmodern}       % tt
...
% Customisation for tcolorbox, the settings make it look similar to the other blocks
\tcbset{colback=white, colframe=dtured, fonttitle=\bfseries}
\tcbset{left=0.25em, right=0.25em, top=0.25em, bottom=0.25em, middle=0.25em}
...
\frame{
	...
	\begin{columns}[T, onlytextwidth]
		\begin{column}{.475\textwidth}
			\begin{tcolorbox}[title={Framed blocks}]\raggedright
				Use the \texttt{tcolorbox} package to get a calm and customizable block.
			\end{tcolorbox}
		\end{column}
		
		\begin{column}{.475\textwidth}
		% You can create colourful boxes with the colframe option. 
		% colframe=[dtured|dtugrey|dtudarkgreen|dtudarkblue|dtuorange|dtuviolet|ipugreen]
			\begin{tcolorbox}[colframe=dtudarkgreen, title={Custom block}]\raggedright
				The package has many options to change the appearance of the blocks.
				\tcblower
				Lower part: a i j and Q J R I.
			\end{tcolorbox}
		\end{column}
	\end{columns}
...
\frame{
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \texttt{\textbackslash usepackage[nofligs]\{verdana\} \% Verdana}
			\item \texttt{\textbackslash usepackage\{arev\} \% Vera Sans, Verdana clone with maths}
			\item \texttt{\textbackslash usepackage\{newtx, newtxmath\} \% Times (text and math)}
		\end{itemize}
		\item DTU recommends Verdana but this font type is not available by default on all operating systems. You can use Vera Sans from the \texttt{arev} package or install Verdana:
		\begin{itemize}
			\item \textbf{Windows:} include the \texttt{verdana} package in the preamble and MiKTeX will do the rest. Remember to set the \texttt{\textbackslash usepackage[nofligs]\{verdana\}} option due to missing ligatures in new versions of Microsoft's Verdana font.
			\item \textbf{Linux and OS X:} before including the \texttt{verdana} package in your preamble, run the \texttt{install\_fonts.sh} script from our package. The old versions of Verdana include more ligatures and no special option has to be set.
		\end{itemize}
	\end{itemize}        
}
